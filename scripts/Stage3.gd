...
@onready var combat3_trigger: Area2D = $Level/CombatTrigger3
@onready var combat4_trigger: Area2D = $Level/CombatTrigger4
...
# ─── Spawn positions (expanded ~5000px world) ───────────────────────────────
const PATROL_SPAWNS  := [Vector2(180, 165), Vector2(310, 165), Vector2(440, 165)]
# Combat 1 – entry hall (x~460)
const C1_SPAWNS      := [Vector2(540, 165), Vector2(650, 165), Vector2(760, 165)]
# Master Key drops after C1 clears
const MASTER_KEY_POS := Vector2(640, 165)
# Combat 2 – mid-hall (x~1050), before locked door (x~1480)
const C2_SPAWNS      := [Vector2(1100, 165), Vector2(1200, 165), Vector2(1300, 165), Vector2(1380, 165)]
# Combat 3 – post-door zone (x~1900)
const C3_SPAWNS      := [Vector2(1950, 165), Vector2(2070, 165), Vector2(2190, 165), Vector2(2310, 165)]
# Combat 4 – upper corridor (x~2650)
const C4_SPAWNS      := [Vector2(2700, 165), Vector2(2820, 165), Vector2(2940, 165), Vector2(3060, 165), Vector2(3180, 165)]
# Staircase enemies (x~3000)
const STAIR_SPAWNS   := [Vector2(3120, 165), Vector2(3250, 150), Vector2(3370, 132)]
# Boss arena (x~3900)
const BOSS_POS       := Vector2(4200, 160)

var has_master_key := false
var door_unlocked := false
var combat1_done := false
var combat2_done := false
var combat3_done := false
var combat4_done := false
...
var combat3_enemies: Array[Node] = []
var combat4_enemies: Array[Node] = []
...
func _ready() -> void:
	...
	combat3_trigger.body_entered.connect(_on_combat3_trigger)
	combat4_trigger.body_entered.connect(_on_combat4_trigger)
...
func on_master_key_collected() -> void:
	...
	if hud:
		hud.set_key(true)
	print("🔑 Master Key collected! Unlocking door...")
	locked_door_zone.set_deferred("monitoring", true)

# ─── Locked Door ──────────────────────────────────────────────────────────────
func _on_locked_door_zone(body: Node) -> void:
	if not body.is_in_group("player") or not has_master_key or door_unlocked:
		return
	door_unlocked = true
...
# ─── Combat 3 ────────────────────────────────────────────────────────────────
func _on_combat3_trigger(body: Node) -> void:
	if not body.is_in_group("player") or combat3_done:
		return
	combat3_trigger.set_deferred("monitoring", false)
	combat3_done = true
	print("⚔️ Combat Area 3!")
	var wave := [TANK_SCENE, RANGED_SCENE, RUSHER_SCENE, TANK_SCENE]
	for i in range(C3_SPAWNS.size()):
		var e = wave[i % wave.size()].instantiate()
		$Level.add_child(e)
		e.global_position = C3_SPAWNS[i]
		combat3_enemies.append(e)
		if e.has_signal("died"):
			e.died.connect(func(): _on_c3_enemy_died(e))
		if hud and hud.has_method("track_enemy"):
			hud.track_enemy(e)

func _on_c3_enemy_died(e: Node) -> void:
	add_score(180)
	if e and is_instance_valid(e):
		_drop_loot(e.global_position)
	combat3_enemies.erase(e)

# ─── Combat 4 ────────────────────────────────────────────────────────────────
func _on_combat4_trigger(body: Node) -> void:
	if not body.is_in_group("player") or combat4_done:
		return
	combat4_trigger.set_deferred("monitoring", false)
	combat4_done = true
	print("⚔️ Combat Area 4!")
	var wave := [RUSHER_SCENE, TANK_SCENE, RANGED_SCENE, RUSHER_SCENE, TANK_SCENE]
	for i in range(C4_SPAWNS.size()):
		var e = wave[i % wave.size()].instantiate()
		$Level.add_child(e)
		e.global_position = C4_SPAWNS[i]
		combat4_enemies.append(e)
		if e.has_signal("died"):
			e.died.connect(func(): _on_c4_enemy_died(e))
		if hud and hud.has_method("track_enemy"):
			hud.track_enemy(e)

func _on_c4_enemy_died(e: Node) -> void:
	add_score(200)
	if e and is_instance_valid(e):
		_drop_loot(e.global_position)
	combat4_enemies.erase(e)
...
func _on_stair_trigger(body: Node) -> void:
	...
	var wave := [TANK_SCENE, RANGED_SCENE, TANK_SCENE]
